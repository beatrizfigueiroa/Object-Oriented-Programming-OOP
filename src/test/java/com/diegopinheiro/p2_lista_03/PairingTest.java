/**
 * @author Diego Pinheiro <diegompin@gmail.com>
 */
package com.diegopinheiro.p2_lista_03;

import static org.junit.Assert.fail;

import org.junit.Assert;
import org.junit.Test;

public class PairingTest {

	private static String MESSAGE = "Cannot obtain pairs with different sizes!";

	@Test
	public void test01getPairsZero() {
		String s1 = "";
		String s2 = "";
		try {
			Pairing pairing = new Pairing(s1, s2);
			Pair[] actual = pairing.getPairs();
			Pair[] expected = new Pair[0];
			Assert.assertArrayEquals(expected, actual);
		} catch (Exception e) {
			fail(e.getMessage());
		}

	}

	@Test
	public void test02PairsZero() {
		String s1 = "";
		String s2 = "";
		try {
			Pairing pairing = new Pairing(s1, s2);
			String actual = pairing.toString();
			String expected = "";
			Assert.assertEquals(expected, actual);
		} catch (Exception e) {
			fail(e.getMessage());
		}
	}

	@Test
	public void test03getPairsOne() {
		String s1 = "Person 1";
		String s2 = "Person 2";
		try {
			Pairing pairing = new Pairing(s1, s2);
			Pair[] actual = pairing.getPairs();
			Pair pairOne = new Pair("Person 1", "Person 2");
			Pair[] expected = new Pair[1];
			expected[0] = pairOne;
			Assert.assertArrayEquals(expected, actual);
		} catch (Exception e) {
			fail(e.getMessage());
		}
	}

	@Test
	public void test04printPairsOne() {
		String s1 = "Person 1";
		String s2 = "Person 2";
		try {
			Pairing pairing = new Pairing(s1, s2);
			String actual = pairing.toString();
			String expected = "(Person 1, Person 2)";
			Assert.assertEquals(expected, actual);
		} catch (Exception e) {
			fail(e.getMessage());
		}

	}

	@Test
	public void test05threePairings() {
		String s1 = "Person 1, Person 3, Person 5";
		String s2 = "Person 2, Person 4, Person 6";

		try {
			Pairing pairing = new Pairing(s1, s2);
			String actual = pairing.toString();
			String expected = "(Person 1, Person 2), " + "(Person 3, Person 4), " + "(Person 5, Person 6)";
			Assert.assertEquals(expected, actual);
		} catch (Exception e) {
			fail(e.getMessage());
		}

	}

	@Test
	public void test06threePairingsDifferentSizesLeftGreater() {
		String s1 = "Person 1, Person 3, Person 5";
		String s2 = "Person 2, Person 4";

		try {
			new Pairing(s1, s2);
			fail(MESSAGE);
		} catch (Exception e) {
			Assert.assertEquals(MESSAGE, e.getMessage());
		}

	}

	@Test
	public void test07threePairingsDifferentSizesRightGreater() {
		String s1 = "Person 1, Person 3";
		String s2 = "Person 2, Person 4, Person 6";
		try {
			new Pairing(s1, s2);
			fail(MESSAGE);
		} catch (Exception e) {
			Assert.assertEquals(MESSAGE, e.getMessage());
		}

	}
}
